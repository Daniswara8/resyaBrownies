...
@section('content')
    ...
    <div class="container-fluid mt-3">
        <div class="row">
            <h3 class="fs-1 text-center">Pesanan Dikonfirmasi</h3>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex flex-column row-gap-3 align-items-center">
                                        <form id="confirmSiap-{{ $order->id }}"
                                            action="{{ route('admin.listSiapAmbil.store', $order->id) }}" method="POST">
                                            @csrf
                                            <button type="button" class="btn btn-kemas"
                                                onclick="confirmSiap({{ $order->id }})">
        ...
        function confirmSiap(orderId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    document.getElementById(`confirmSiap-${orderId}`).submit();
                }
            });
        }
