...
@section('content')
    ...
    <div class="container-fluid mt-3">
        <div class="row">
            <h3 class="fs-1 text-center">Pesanan Pending (Belum Dikonfirmasi)</h3>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex flex-column row-gap-3 align-items-center">
                                        <form id="confirmKemas-{{ $order->id }}"
                                            action="{{ route('admin.listKonfirmasi.store', $order->id) }}" method="POST">
                                            @csrf
                                            <button type="button" class="btn btn-kemas"
                                                onclick="confirmKemas({{ $order->id }})">
...
@section('sweetAlert')
    <script>
        function confirmKemas(orderId) {
            Swal.fire({
                title: "Apakah yakin ingin mengemas produk ini?",
                ...
                if (result.isConfirmed) {
                    document.getElementById(`confirmKemas-${orderId}`).submit();
                }
            });
        }
